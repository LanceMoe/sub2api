...
func BuildUserAgent(version string) string {
	if normalized := NormalizeUserAgentVersion(version); normalized != "" {
		return fmt.Sprintf("antigravity/ide/%s (aidev_client; os_type=windows; arch=amd64)", normalized)
	}
	return fmt.Sprintf("antigravity/ide/%s (aidev_client; os_type=windows; arch=amd64)", defaultUserAgentVersion)
}
